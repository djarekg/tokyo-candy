import type { ComponentProps, FC } from 'react';
import type { IconProps } from './icon-props';

type ContactFilledProps = ComponentProps<'svg'> &
  IconProps & {
    fill?: string;
    strokeWidth?: number;
    strokeColor?: string;
  };

const ContactFilledIcon: FC<ContactFilledProps> = ({
  className,
  fill = 'currentColor',
  size = 24,
  strokeWidth = 1.5,
  strokeColor = '#222',
}) => {
  return (
    <svg
      data-icon="contact-filled-icon"
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
      className={`app-icon app-contact-filled-icon ${className}`}
      fill="currentColor"
      width={`${size}px`}
      height={`${size}px`}>
      <title>ContactFilled</title>
      <path
        fill={fill}
        strokeWidth={strokeWidth}
        stroke={strokeColor}
        d="M14 11h7V6h-7zm3.5-1.25L15 8V7l2.5 1.75L20 7v1zM2 21q-.825 0-1.412-.587T0 19V5q0-.825.588-1.412T2 3h20q.825 0 1.413.588T24 5v14q0 .825-.587 1.413T22 21zm7-7q1.25 0 2.125-.875T12 11t-.875-2.125T9 8t-2.125.875T6 11t.875 2.125T9 14m-6.9 5h13.8q-1.05-1.875-2.9-2.937T9 15t-4 1.063T2.1 19"
      />
    </svg>
  );
};

export default ContactFilledIcon;
